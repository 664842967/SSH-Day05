...
//        session.save(user);
        ...
        *
        * saveOrUpdate方法先取检查当前对象的主键id是否存在
        * 如果存在执行非主键的update更新操作
        * 如果当前实体类对象的主键id不存在,则执行的是save操作
        * ,即insert插入*/
//        user.setId(7);
    ...
    public void delete() {
        ...
        * 则不进行任何操作*/
        user.setId(4);//设置要删除的id
        session.delete(user);

        /*session自带的delete方法只根据id进行删除,不考虑其他删除条件*/
...
//
//        /*设置sql语句中问好所对应的值*/
//        query.setString(0,"李四");//第一个问好的替换值
//        query.setString(1,"123");//第二个问好的替换值
